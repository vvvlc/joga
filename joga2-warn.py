#!/usr/bin/env /home/vitek/robozonky/joga/joga3/bin/python
# -*- coding: utf-8 -*-
# - *- coding: ISO-8859-2 -*-
...
def scrap_current_week_to_csv(outfile, dtnow,now, cdate, append=False):
    ...
    r = s.post(url="https://rezervace.dum-jogy.cz/rs/login/run", data={"username":"vvvlcek","password": "adgv123"})
    ...
    for day in soup.find_all('div',{'class':re.compile(lekce_wrapper_pattern)}):
        ...
        for i in day.find_all('div', {'class': 'jedna-lekce-vypis'}):
            ...
            i.find('div',{'class':'rezervace_info_do_kalendare'})
            ...
            if len(tmp)>0 and i.find('div',{'class':'rezervace_info_do_kalendare'}):
                #tmp=tmp.replace(')','').replace('(','').split("/")
                matchobj=re.match(r".*[^\d]*((\d+)\s*\/\s*(\d+))",tmp.replace(')','').replace('(',''))
                if matchobj:
                    rezervace=matchobj.group(2)
                    kapacita=matchobj.group(3)
                    lekcedt=datetime.datetime.strptime(f"{datum} {od}", "%d.%m.%Y %H:%M").replace(tzinfo=timezone('CET'))

                    if lekcedt.hour<=7 or lekcedt.weekday() in (5,6):
                       #hlaseni 2hodiny predem
                       hlaseni=datetime.timedelta(minutes=120)
                    else:
                       #hlaseni 30 minut predem
                       hlaseni=datetime.timedelta(minutes=30)
                    if not (int(rezervace)>9 and lekcedt-dtnow<hlaseni and (lekcedt-dtnow).total_seconds()>0):
                       continue
                    ...
                    csv_line = [snow, datum, popis, od, do, rezervace, kapacita, instruktor, typ, aktivita]
                            #replace, by -
                    print(",".join([fld.replace(',','-') for fld in csv_line]), file=sys.stderr)
                else:
                    rezervace="N/A"
                    kapacita="N/A"
...
if __name__ == '__main__':
    ...
    #print(cdate)
    scrap_current_week_to_csv(outfilename, now, snow,cdate, append=True)
