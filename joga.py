...
def scrap_current_week_to_csv(outfile, now, aktivita, append=False):
    ...
    #find dnesni datum
    datum=soup.find('td', { "class" : "tdDnesniDatum" }).text.split(' ', 1)[0]
    cas=soup.find('th', { "class" : "tdRezervaceKalendarPrvni" }).text
    #find aktualni cas
    matchObj=re.match( r'.*aktualizace:\s*(\d{1,2}:\d{1,2}):\d{1,2}.*', cas, re.M|re.I)
    if matchObj:
        now = datum+" "+matchObj.group(1).strip()
    else:
        now="n/a"
        ...
        for item in pole:
            ...
            if day >= 0:
                ...
                if '/' in akce: #radek ve formatu: OD - DO / POPIS
                    #akce: ZÁSKOK Kateřina Kostková za Veroniku Brožovou 18:00 - 19:30 / Hot jóga
                    #akce: 19:45 - 21:15 / Hot jóga                    
                    #akce: 16:10 - 17:10 / Jóga pro zdravá záda - vhodná pro začátečníky
                    #akce: 9:15 - 10:15 / Hatha jóga
                    #akce: 17:20 - 18:50 zaskok / Ashtanga joga
                    #testovaci link
                    #http://fiddle.re/npecwd
                    matchObj = re.match( r'akce:\s*(.*)\s+(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})\s*.*/(.*)', akce, re.M|re.I)
                    if matchObj:
                        od = matchObj.group(2).strip()
                        do = matchObj.group(3).strip()
                        popis = matchObj.group(4).strip()
                    else:
                        od = "N/A"
                        do = "N/A"
                        popis = akce                    
